...
/* ------------------------------------------------------------ */
#define TAGSIZE 0.092
#define FX   1122.04673
#define FY   1124.22424
#define CX   641.914219
#define CY   334.365895
...
/**
 * Given a local minima of the pose error tries to find the other minima.给定一个姿态误差的极小值，试图去寻找另一个极小值
 */
...
/**
 * @param v Image points on the image plane.像素坐标系
 * @param p Object points in object space.世界坐标系
...
double orthogonal_iteration(matd_t **v, matd_t **p, matd_t **t, matd_t **R, int n_points, int n_steps)
{
    ...
    matd_scale_inplace(p_mean, 1.0 / n_points);//求均值

    matd_t **p_res = malloc(sizeof(matd_t *) * n_points);
    for (int i = 0; i < n_points; i++)
    {
        p_res[i] = matd_op("M-M", p[i], p_mean);//求与均值的差
    }
    ...
    for (int i = 0; i < n_points; i++)
    {
        F[i] = calculate_F(v[i]);//计算每个点的坐标投影算子
        matd_add_inplace(avg_F, F[i]);
    }
    matd_scale_inplace(avg_F, 1.0 / n_points);
    matd_t *I3 = matd_identity(3);//创建一个单位矩阵
    matd_t *M1 = matd_subtract(I3, avg_F);//减法
    matd_t *M1_inv = matd_inverse(M1);//逆矩阵
    matd_destroy(avg_F);
    matd_destroy(M1);
...
{
    // Note that every variable that we compute is proportional to th+e scale factor of H.
    ...
    // now recover [R02 R12 R22] by noting that it is the cross product of the other two columns. //叉积
    ...
    if (1) {
        ...
        matd_svd_t svd = matd_svd(R);//SVD分解
        matd_destroy(R);
    ...
    int nIters)
{
    ...
    estimate_pose_for_tag_homography(info, solution1);//采用DLT进行简单位姿估计

    *err1 = orthogonal_iteration(v, p, &solution1->t, &solution1->R, 4, nIters);//正交迭代优化

    solution2->R = fix_pose_ambiguities(v, p, solution1->t, solution1->R, 4);//寻找潜在的极小值
    if (solution2->R)//若存在这个极小值
    {
