...
void apriltag_detector_init(apriltag_detector_t *ptd, apriltag_family_t *ptf)
{
    ...
    ptd->qtp.deglitch = 0;
    ptd->qtp.min_white_black_diff = 20;
...
zarray_t * apriltag_detector_detect(apriltag_detector_t *td, image_u8_t *im_orig, timeprofile_t *tp)
{
    ...
    if (td->quad_sigma != 0) {
        ...
        float sigma = fabsf((float) td->quad_sigma); //sigma为什么
        ...
        if (ksz > 1) {
            ...
            if (td->quad_sigma > 0) {
                ...
                // SHARPEN the image by subtracting the low frequency components.    锐化
...
void apriltag_duty()
{
    ...
    if (zarray_size(detections) == 0) {
    	xil_printf("apriltag_flag = 0\r\n");
    }


    if (zarray_size(detections) != 0) {
    	xil_printf("apriltag_flag = 1\r\n");
    }
    ...
    for (int i = 0; i < zarray_size(detections); i++) {
        ...
        double Rx, Ry, Rz;// 分别对应滚转角，俯仰角，偏航角

        Rx = radians_to_degrees(atan2(MATD_EL(pose.R, 2, 1), MATD_EL(pose.R, 2, 2)));
        Ry = radians_to_degrees(asin(-MATD_EL(pose.R, 2, 0)));
        Rz = radians_to_degrees(atan2(MATD_EL(pose.R, 1, 0), MATD_EL(pose.R, 0, 0)));

        printf("err is %f \r\n", err);
        printf("tx: %f, ty %f, tz %f, Rx %f, Ry %f, Rz %f\r\n", MATD_EL(pose.t, 0, 0), MATD_EL(pose.t, 1, 0), MATD_EL(pose.t, 2, 0), Rx, Ry, Rz);
