...
int sys_init(void) {
	...
	for (i = 0; i < DISPLAY_NUM_FRAMES; i++)
	{
		...
		pthresh_buf[i] = thresh_buf[i];

		memset(color_buf[i], 1, FRAME_SIZE);
		memset(gray_buf[i], 1, GRAY_FRAME_SIZE);
		memset(thresh_buf[i], 255, THRESH_FRAME_SIZE);
	}
	...
	/*Reset sensor ov5640*/
	XGpioPs_WritePin(&Gpio, GPIO_CAM_PIN, 0); //PWDN拉高，RESETB_拉低
	usleep(1000000); //延时1s
	XGpioPs_WritePin(&Gpio, GPIO_CAM_PIN, 1) ; // PWDN拉低,，RESETB拉高
	usleep(1000000);//延时1s
	...
	Status = DpdmaVideoExample(&RunCfg, pthresh_buf[0]);
	if (Status != XST_SUCCESS) {
	...
	if (Status != XST_SUCCESS) {
		xil_printf("vdma_write_init_0 Failed\r\n");
		return XST_FAILURE;
	}
	...
	if (Status != XST_SUCCESS) {
		xil_printf("vdma_write_init_1 Failed\r\n");
		return XST_FAILURE;
	}
	...
	/* Start Vdma2 transmission*/

	Status = vdma_write_init_2(VDMA_DEVICE_ID_2, TH_VIDEO_COLUMNS, TH_VIDEO_ROWS, THRESH_STRIDE, (u32)pthresh_buf[0], THRESH);
	if (Status != XST_SUCCESS) {
		xil_printf("vdma_write_init_2 Failed\r\n");
		return XST_FAILURE;
	}
