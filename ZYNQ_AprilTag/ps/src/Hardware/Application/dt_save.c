...
/*Framebuffers for threshhold picture*/
u8 thresh_buf[DISPLAY_NUM_FRAMES][THRESH_FRAME_SIZE] __attribute__ ((aligned(256)));
u8 *pthresh_buf[DISPLAY_NUM_FRAMES];
...
void threshhold_monitor (){
	thr_save_flag = 1;

	for (int y = 0; y < 640; y++) {
		for (int x = 0; x < 320; x++) {
			color_buf[0][y*2*640*4 + 4*x + 0] = thresh_buf[(wr_the_index+2)%3][y*640 + x];
			color_buf[0][y*2*640*4 + 4*x + 1] = thresh_buf[(wr_the_index+2)%3][y*640 + x];
			color_buf[0][y*2*640*4 + 4*x + 2] = thresh_buf[(wr_the_index+2)%3][y*640 + x];
		}
	}

	FrameBuffer.Address = (INTPTR)color_buf[0];
