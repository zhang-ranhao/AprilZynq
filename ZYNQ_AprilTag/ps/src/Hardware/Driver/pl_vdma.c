...
static void ImageReadingControl (void *CallbackRef)
{
	...
	XAxiVdma_StartParking((XAxiVdma*)CallbackRef, wr_the_index, XAXIVDMA_WRITE);
}

static void ImageReadingControl2 (void *CallbackRef)
{
	/*Color image updating*/
	/*
	if (stFsm.stCurState == IDLE)
 ...
 *
 ******************************************************************************/
static void WriteCallBack2(void *CallbackRef, u32 Mask)
{
	...
	if (Mask & XAXIVDMA_IXR_FRMCNT_MASK)
	{
		Xil_DCacheInvalidate();   //invalidate the cache
		wr2_fps++;
		ImageReadingControl2(CallbackRef);
	}
}
...
int vdma_write_init_2(short DeviceID,short HoriSizeInput,short VertSizeInput,short Stride, u32 FrameStoreStartAddr, u8 video_format)
{
	...
	Status = XAxiVdma_CfgInitialize(&Vdma_2, Config, Config->BaseAddress);
	if (Status != XST_SUCCESS) {
		xil_printf("XAxiVdma_CfgInitialize failure\r\n");
		return XST_FAILURE;
	}
	...
	Status = XAxiVdma_DmaConfig(&Vdma_2, XAXIVDMA_WRITE, &WriteCfg);
	if (Status != XST_SUCCESS) {
			xdbg_printf(XDBG_DEBUG_ERROR,
				"Read channel config failed %d\r\n", Status);
	...
	/* Set General Callback for Sensor Vdma */
/*
	XAxiVdma_SetCallBack(&Vdma_2, XAXIVDMA_HANDLER_GENERAL, WriteCallBack2, (void *)&Vdma_2, XAXIVDMA_WRITE);

	Status = SetupIntrSystem(&Vdma_2, VDMA_INTR_WRITE_2);
	if (Status != XST_SUCCESS) {
		xil_printf("Setup interrupt system failed %d\r\n", Status);
		return XST_FAILURE;
	}
...
/*
	...
	Status = XAxiVdma_SetFrameCounter(&Vdma_2, &FrameCfg);
	if (Status != XST_SUCCESS) {
		xil_printf(
			"Set frame counter failed %d\r\n", Status);
	...
	XAxiVdma_IntrEnable(&Vdma_2, XAXIVDMA_IXR_FRMCNT_MASK, XAXIVDMA_WRITE);
*/

	for (int i = 0; i < DISPLAY_NUM_FRAMES; i++) {
		WriteCfg.FrameStoreStartAddr[i] = FrameStoreStartAddr + i * THRESH_FRAME_SIZE;
	}
	Status = XAxiVdma_DmaSetBufferAddr(&Vdma_2, XAXIVDMA_WRITE, WriteCfg.FrameStoreStartAddr);
	if (Status != XST_SUCCESS) {
			xdbg_printf(XDBG_DEBUG_ERROR,"Write channel set buffer address failed %d\r\n", Status);
			return XST_FAILURE;
	}

//	XAxiVdma_StartParking(&Vdma_2, 0, XAXIVDMA_WRITE);

	Status = vdma_write_start(&Vdma_2);
	if (Status != XST_SUCCESS) {
		   xil_printf("error starting VDMA..!");
		   return Status;
	}
